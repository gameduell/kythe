...
(defn drop-msg [x] ;raw msg from js
  (fn [x]
    (.log js/console "sent")
    (.log js/console x)
    (put! comm (om/value x))))
...
;  (go (loop []
;        (<! (async/timeout (* 1000 (rand-int 5) (.log js/console "sent random msg"))))
;        (>! channel publish-value)
;        (recur))))

; start putting stuff on the channels
;;(randomly-constantly comm   "comm <- msg")


(defn print-state[state]
  (do
  (.log js/console (:file-to-view state))
  (.log js/console (:xrefs-to-view state))
  (.log js/console (:hover state))
  ))
...
(defn kythe-app [state owner]
  (reify
    ...
    om/IWillMount
    (will-mount [_]
      ;;listen for events from js from outside
...
(defn print-kstate []
  ;;little helper to output state
  (.log js/console kythe-state))

(defn set-state [ticket]
  (.log js/console ticket)
  (om/transact! kythe-state :current-file (fn [s] (.log js/console s)))
  (.log js/console (om/get-state kythe-app :current-file)))
  ;;(replace-state! kythe-state :current-file))
